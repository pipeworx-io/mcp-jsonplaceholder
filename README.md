...
`tools/list` at `https://gateway.pipeworx.io/jsonplaceholder/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/jsonplaceholder_get_posts \
  -H 'Content-Type: application/json' \
  -d '{"limit":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/jsonplaceholder_get_posts`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
